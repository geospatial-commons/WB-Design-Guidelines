...
> **Note:** These differ from the dashboard RAG tokens (`rag-red`, `rag-amber`, `rag-green`) in `design-guide/colors.md`, which are tuned for UI components on white backgrounds. Use the UK Gov values above for filled table cells; use the dashboard tokens for badge/pill UI elements.
...
- [`palettes.json`](palettes.json) — machine-readable tokens
- [`chart-guidelines.md`](chart-guidelines.md) — axis styling, gridlines, legends, tooltips
- [`../design-guide/colors.md`](../design-guide/colors.md) — WB brand colors (separate from data viz)
